"""v9 Task 5: 55-point cv-array EP sweep with REAL OTA at Vbn=1.0V (best bias from Task 1).

Use the EP testbench with silicon-matched OTA NFET sizing.
"""
...
* AC coupling V1 -> Mf gate
Cac     V1     vin_p   14.112p
RbiasIn vin_p  Vbmid_  1e7

* === Real silicon-matched NMOS-only OTA (single-stage SF) ===
* Mf source-follower (W_total=400, L=0.15)
XM_sf  VDPWR vin_p Vfo  GND sky130_fd_pr__nfet_01v8 W=5 L=0.15 nf=8 m=10
* M_tail tail (W=200, L=0.5)
...
.nodeset V(Vfo)=0.1 V(vin_p)=0.9 V(V1)=0.0 V(V2_in)=0.0
...
def main():
    ...
    print(f"Running {len(eps_list)} eps with Vbn=1.0V real OTA")
    results = []
    ...
    with open(f"{WORKDIR}/results.json", 'w') as f:
        json.dump({"results": results,
                   "summary": {"slope": float(slope) if 'slope' in dir() else None,
                               "r2": float(r2) if 'r2' in dir() else None,
                               "n_resolved": len(in_w)}}, f, indent=2)
